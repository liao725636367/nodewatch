...
func RunCmd(str string, limitSecond ...int) (result string, err error) {
	t := 10
	if len(limitSecond) > 0 {
		t = limitSecond[0]
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*time.Duration(t)) //命令最长十秒要返回
	defer cancel()
